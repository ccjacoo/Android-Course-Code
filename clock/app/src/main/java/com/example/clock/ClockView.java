...
public class ClockView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        setTimes();
        drawCirclePoint(canvas); //圆点
        drawCircle(canvas);    //圆
        drawDial(canvas);     //画刻度和数字
    ...
    //安卓坐标系与数学坐标系不同的地方是X轴是相反的，所以为了调整方向，需要将角度+270度 , Hour角度应+150度
    private void drawPointer(Canvas canvas) {
        canvas.translate(circleX, circleY);
        float hourX = (float) Math.cos(Math.toRadians(hour*30+150+hour_angle(minute)))*circleRadius*0.5f;
        float hourY = (float) Math.sin(Math.toRadians(hour*30+150+hour_angle(minute)))*circleRadius*0.5f;
    ...
    //根据分针现在处于的位置来调整时针的微角度
    public int hour_angle(int minute){
        return (int)(minute/15)*8;
    }
    ...
    private void setTimes() {
        ...
        hour = getTimes(date, Calendar.HOUR_OF_DAY) % 12;
    }
